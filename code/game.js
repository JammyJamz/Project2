...
Projectile.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle === "bounce"){
    ...
    console.log("jump more then a kid with ADHD");
    //anti gravity area
  }
  if (obstacle === "slip"){
    if (playerXSpeed != 0.5){
      playerXSpeed = 0.5;
      gravity = 200;
      console.log("time to suffer worse then the DMV");
      //horrifyingly badly named variable
    }
  }
  else if (moreObstacles === "slip"){
    setTimeout(function() {
      playerXSpeed = 7;
      gravity = 30;
    },250);
  }
  var darkness = document.getElementsByClassName("background");
  if (obstacle === "dark"){
    if (darkness[0].style.background != "black"){
      darkness[0].style.background = "black";
      console.log("I want my mommy! I am scared!")
    }
  }
  else if (moreObstacles === "dark"){
    darkness[0].style.background = '#730046';

}
  moreObstacles = obstacle;
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "projectile") {
